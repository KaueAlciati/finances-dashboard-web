...
const KEY_PREF  = 'fincontrol_prefs_v1'; // { theme, currency }

/* ======================================
   Estado / Preferências / Utils
====================================== */
let txs = [];
let cats = [];
let lineChart, pieChart;

// carrega prefs com defaults
const PREFS = (() => {
...
// formatadores centralizados (locale fixo pt-BR, moeda dinâmica)
const FC_FORMAT = {
  money(n) {
...
const avatarEl      = document.querySelector('.set-card-head .avatar'); // círculo "AS" no card Perfil
...
function renderProfile() {
  ...
  const emailFromLogin =
    localStorage.getItem('fincontrol_user') ||
    userObj.email ||
    'usuario@exemplo.com';

  const baseName = userObj.name || (emailFromLogin.split('@')[0] || 'Usuário');
  ...
  // Inputs em Configurações
  if (settingName)  settingName.value  = baseName;
  if (settingEmail) settingEmail.value = emailFromLogin;
  ...
  if (userObj.avatar) {
    ...
    if (avatarEl) {
      avatarEl.style.backgroundImage = `url(${userObj.avatar})`;
...
function refreshCards() {
  ...
  const monthIncome  = txs
    .filter(t => t.type === 'income'  && isoToDate(t.date) >= from && isoToDate(t.date) <= to)
    .reduce((s, t) => s + Number(t.value), 0);

  const monthExpense = txs
    .filter(t => t.type === 'expense' && isoToDate(t.date) >= from && isoToDate(t.date) <= to)
    .reduce((s, t) => s + Number(t.value), 0);
...
function refreshLast() {
  tbodyLast.innerHTML = '';
  const last = [...txs]
    .sort((a, b) => isoToDate(b.date) - isoToDate(a.date))
    .slice(0, 8);

  if (!last.length) {
    const tr = document.createElement('tr');
    tr.innerHTML =
      `<td colspan="5" style="color:#7d8a8a;padding:12px">Sem transações ainda.</td>`;
...
function refreshCharts() {
  ...
  lineChart = new Chart(document.getElementById('lineChart').getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        data: values,
        borderColor: '#0ea564',
        backgroundColor: 'rgba(14,165,100,.12)',
        fill: true,
        tension: .35,
        pointRadius: 2
      }]
  ...
  const pLabels = Object.keys(byCat).map(k =>
    (cats.find(c => c.id === k) || { name: k }).name
  );
  const pValues = Object.values(byCat);
  const pColors = Object.keys(byCat).map(k =>
    (cats.find(c => c.id === k) || { color: '#bbb' }).color
  );
  ...
  pieChart = new Chart(document.getElementById('pieChart').getContext('2d'), {
    ...
    data: {
      labels: pLabels,
      datasets: [{ data: pValues, backgroundColor: pColors }]
    },
    options: {
      plugins: {
...
function refreshNotifications() {
  ...
  budgets.forEach(b => {
    ...
    if (pct >= 80) {
      ...
      div.className = 'note';
      div.innerHTML =
        `⚠️ Seu orçamento para <b>${cat.name}</b> está em <b>${pct.toFixed(0)}%</b>.`;
  ...
  if (!notifBox.children.length) {
    notifBox.innerHTML =
      `<div class="muted" style="padding:8px 0">Nenhuma notificação no momento.</div>`;
  }
}
...
/* ======================================
   Avatar Picker (usa av1..av6.png em /img)
====================================== */
  ...
  avatarEl.addEventListener('click', () => {
    ...
    AVATARS.forEach(src => {
      ...
      img.addEventListener('click', () => {
        const current = JSON.parse(localStorage.getItem(KEY_USER) || '{}');
        const next = { ...current, avatar: src };
        localStorage.setItem(KEY_USER, JSON.stringify(next));
        renderProfile();
      });
      picker.appendChild(img);
    });
    avatarEl.parentElement.appendChild(picker);
  });
})();

/* ======================================
   Tema Claro/Escuro + Ícones Sidebar
====================================== */
(function initTheme() {
  ...
  function updateNavIcons(theme) {
    navIcons.forEach((img, i) => {
      const conf = iconMap[i];
      if (!conf) return;
  ...
  function apply(theme) {
    ...
    document.dispatchEvent(
      new CustomEvent('fc:themechange', { detail: { theme } })
    );
  }
...
/* ======================================
   Moeda (select) - só currency
====================================== */
(function initCurrency() {
  if (!selCurrency) return;
  selCurrency.value = PREFS.currency;
  ...
  selCurrency.addEventListener('change', () => {
    ...
    refreshLast();
    refreshCharts();
    // se outras páginas ouvirem:
    document.dispatchEvent(
      new CustomEvent('fc:currencychange', { detail: { currency: PREFS.currency } })
    );
  });
})();
